...
sale.addEventListener("click", () => {
  ...
  if (
    ...
    if (changeResult.status === "CLOSED") {
      change.innerText += ` ${formatChange(changeResult.change)}`;
    }
  } else {
...
const getChange = (changeDue, cid) => {
  ...
  for (let i = currencyUnits.length - 1; i >= 0; i--) {
    ...
    let unitInDraw = cid[i][1];

    if (unitValue <= remainingChange && unitInDraw > 0) {
      let amountFromUnit = 0;
      while (remainingChange >= unitValue && unitInDraw > 0) {
        remainingChange = (remainingChange - unitValue).toFixed(2);
        unitInDraw -= unitValue;
